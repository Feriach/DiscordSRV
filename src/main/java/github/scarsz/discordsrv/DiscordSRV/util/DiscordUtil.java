...
public class DiscordUtil {
    ...
    public static int purgeChannel(TextChannel channel) {
        if (!DiscordUtil.checkPermission(channel, Permission.MESSAGE_MANAGE)) {
            ...
            else DiscordSRV.getInstance().getPlatform().warning(message);
            return -1;
        }
    ...
    public static void sendMessage(TextChannel channel, String message, int expiration) {
        ...
        if (message.length() > 2000) {
            DiscordSRV.getInstance().getPlatform().warning("Tried sending message with length of " + message.length() + " (" + (message.length() - 2000) + " over limit)");
        ...
        queueMessage(channel, message, m -> {
            if (expiration > 0 && checkPermission(channel, Permission.MESSAGE_MANAGE)) {
                ...
                if (checkPermission(channel, Permission.MESSAGE_MANAGE)) m.deleteMessage().queue(); else DiscordSRV.getInstance().getPlatform().warning("Could not delete message in channel " + channel + ", no permission to manage messages");
            }
        });
